...
module type DOMAIN =
  sig
    ...
    (* an element of type t abstracts a set of mappings from variables /un element de type t est une abstraction d'un ensemble de mappage de variable à des entiers
       to integers
     *)
    type t

    (* initial environment, without any variable //Environement initialse sans aucune variable *) 
    val init: unit -> t
